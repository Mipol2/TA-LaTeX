...
Bab ini merupakan realisasi dari tahap \textit{Prototype} dalam metodologi \textit{Design Thinking} yang telah diuraikan pada Subbab I.5.2. Berdasarkan hasil analisis kebutuhan dan pemilihan solusi pada Bab III, dirancang sistem \textit{Smart Waste Bin} yang terdiri dari perangkat IoT, \textit{middleware}, \textit{database}, dan \textit{dashboard}.

Komponen sistem dibedakan menjadi dua kategori: komponen eksternal berupa perangkat IoT \textit{Smart Waste Bin} dari pihak ketiga, serta komponen internal yang dikembangkan oleh peneliti meliputi \textit{middleware}, \textit{database}, dan \textit{dashboard}. Karena konfigurasi \textit{firmware} bawaan perangkat dirancang untuk skala perkotaan sebagaimana diidentifikasi pada Subbab \ref{sec:analisis-kondisi}, bab ini juga menjelaskan perancangan peningkatan \textit{firmware} yang dilakukan agar perangkat dapat beroperasi optimal di lingkungan kampus.
...
Sistem \textit{Smart Waste Bin} mengimplementasikan alur data yang terstruktur dari pengumpulan data di perangkat IoT hingga penyajian informasi kepada pengguna. Untuk menggambarkan alur data secara sistematis, digunakan \textit{Data Flow Diagram} (DFD) yang terdiri dari dua level: DFD Level 0 (\textit{Context Diagram}) dan DFD Level 1.

DFD Level 0 menggambarkan sistem \textit{Smart Waste Bin} sebagai satu kesatuan proses yang berinteraksi dengan entitas eksternal, sebagaimana ditunjukkan pada Gambar \ref{fig:dfd-level0}.
...
dengan kedalaman \textit{bin} sebesar 1000 mm. Sensor INA219 mengukur tegangan dan arus baterai untuk menghitung \textit{State of Charge} (SoC) menggunakan Persamaan \ref{eq:battery-percent}.
...
Interval pengambilan data diubah dari 8 jam menjadi 2 jam 30 menit berdasarkan dua alasan utama. Pertama, dinamika pembuangan sampah kampus yang sangat dinamis dan terkonsentrasi pada jam-jam tertentu, sehingga interval 8 jam berisiko kehilangan momen kritis pelaporan status. Kedua, kebutuhan resolusi data untuk analisis pola: interval 8 jam hanya menghasilkan sekitar 3 titik data per hari, sedangkan interval 2,5 jam menghasilkan 8 sampai 9 titik data per hari yang cukup untuk menangkap fluktuasi aktivitas kampus secara detail.
