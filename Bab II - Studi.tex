...
Bab ini menyajikan hasil studi literatur sebagaimana ditetapkan pada Subbab I.5.1. Studi literatur dilakukan untuk membangun fondasi teoritis yang mendasari perancangan dan implementasi sistem pemantauan sampah berbasis \textit{Internet of Things} (IoT). Pembahasan mencakup tiga topik utama: pengelolaan sampah dan tantangannya dalam konteks lingkungan kampus, teknologi \textit{Internet of Things} beserta arsitektur dan komponen yang relevan, serta studi terkait sistem pemantauan sampah berbasis \textit{Internet of Things} yang telah dikembangkan sebelumnya.
...
\begin{enumerate}
  ...
  Dalam konteks sistem pemantauan sampah berbasis \textit{Internet of Things}, \textit{Perception Layer} mencakup sensor ultrasonik atau sensor \textit{Time-of-Flight} yang dipasang pada tempat sampah untuk mengukur tingkat kepenuhan secara periodik. Karakteristik penting dari \textit{layer} ini adalah kemampuannya untuk beroperasi dalam berbagai kondisi lingkungan, efisiensi energi untuk perangkat berbasis baterai, dan akurasi dalam pengukuran.
  ...
  Dalam sistem pemantauan sampah berbasis \textit{Internet of Things}, \textit{Network Layer} dapat menggunakan koneksi Wi-Fi atau \textit{cellular network} (seperti 4G/5G) untuk mengirimkan data tingkat kepenuhan sampah dari lokasi tempat sampah ke \textit{server} pusat secara periodik.
  ...
  Dalam konteks sistem pemantauan sampah berbasis \textit{Internet of Things}, \textit{Application Layer} mencakup sistem \textit{database} untuk menyimpan data pembuangan sampah secara terstruktur, serta \textit{dashboard} yang memungkinkan pengelola untuk melihat status tempat sampah dan hasil analisis pola pembuangan.
\end{enumerate}
...
\textit{Analytical Hierarchy Process} (AHP) adalah metode pengambilan keputusan multikriteria yang dikembangkan oleh \textcite{saaty1980ahp} untuk menyusun prioritas di antara beberapa alternatif berdasarkan sejumlah kriteria. AHP bekerja dengan menguraikan masalah ke dalam struktur hierarki (tujuan, kriteria, dan alternatif), lalu melakukan perbandingan berpasangan (\textit{pairwise comparison}) antar elemen untuk menentukan bobot prioritas \autocite{saaty1980ahp}.

Perbandingan berpasangan dilakukan menggunakan skala fundamental Saaty yang bernilai 1 hingga 9, sebagaimana disajikan pada Gambar \ref{fig:skala-saaty}. Pada penelitian ini digunakan nilai ganjil (1, 3, 5, 7, 9) beserta resiprokalnya agar setiap perbandingan memiliki makna verbal yang jelas dan dapat dipertanggungjawabkan.

\begin{figure}[H]
  \caption{Skala Fundamental Saaty}
  \label{fig:skala-saaty}
  \centering
  \includegraphics[width=0.95\textwidth]{images/skala-saaty.png}
\end{figure}

Konsistensi penilaian diukur melalui \textit{Consistency Ratio} (CR). Penilaian dianggap konsisten dan dapat diterima apabila nilai CR tidak melebihi 0,1 (10\%) \autocite{saaty1980ahp}.

% ============================================================
\section{Studi Terkait}
% ============================================================

Bagian ini mengkaji penelitian-penelitian terdahulu yang relevan dengan topik pengembangan sistem pemantauan sampah berbasis \textit{Internet of Things}, dengan fokus pada metode yang digunakan, hasil yang diperoleh, dan \textit{gap} yang menjadi dasar penelitian ini.

\subsection{Sistem Pemantauan Sampah Berbasis \textit{Internet of Things}}
...
\textcite{iotsmartwaste2025urban} mengembangkan sistem manajemen sampah IoT berbiaya rendah yang mengintegrasikan lima sensor (ultrasonik HC-SR04 untuk tingkat kepenuhan, \textit{load-cell} HX711 untuk berat, DHT22 untuk suhu dan kelembapan, sensor gas MQ-135, serta GPS) di sekitar inti Arduino-ESP8266 dengan analitik \textit{cloud} Firebase. Yang relevan untuk penelitian ini adalah metode pengujiannya: sistem diuji melalui \textit{field trial} selama 30 hari pada 12 tempat sampah dengan hasil akurasi tingkat kepenuhan 85\%, \textit{mean absolute error} berat di bawah 3\%, dan deteksi kondisi berbahaya 100\%. Penelitian ini menunjukkan pendekatan pengujian berbasis penempatan lapangan dalam periode tertentu, serupa dengan pendekatan pada penelitian ini.

\textcite{addas2024waste} mengembangkan sistem \textit{Waste Management 2.0} yang memanfaatkan sensor ultrasonik dengan arsitektur konektivitas LoRaWAN dan seluler serta analitik \textit{cloud} untuk pengelolaan sampah skala kota. Sistem ini dikembangkan dan divalidasi melalui eksperimen ekstensif berbasis data dalam jumlah besar, tanpa melibatkan pengujian pengguna (\textit{user testing}). Pendekatan ini menegaskan bahwa pada domain pengembangan sistem pemantauan sampah, validasi umumnya dilakukan terhadap kinerja dan keandalan artefak, bukan terhadap penerimaan pengguna, yang sejalan dengan metodologi \textit{Design Science Research} yang digunakan pada penelitian ini.

\textcite{swe2025sustainability} mengevaluasi akurasi sensor ultrasonik untuk deteksi tingkat kepenuhan dalam kondisi terkontrol, dengan menempatkan sampah pada berbagai tingkat yang telah diketahui (25\%, 50\%, 75\%, 90\%, dan 100\%) dan membandingkan pembacaan sensor terhadap nilai aktual tersebut. Hasilnya menunjukkan konsistensi akurasi dengan \textit{margin of error} dalam rentang $\pm$3\%. Pendekatan pengujian akurasi dengan membandingkan pembacaan sensor terhadap nilai acuan yang diketahui ini menjadi referensi metode bagi evaluasi akurasi sensor pada penelitian ini.

Untuk memberikan gambaran posisi penelitian ini di antara penelitian terdahulu, Tabel \ref{tab:perbandingan-penelitian} menyajikan perbandingan berdasarkan sensor, konektivitas, fokus penelitian, metode pengujian, dan ada tidaknya analisis pola.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{| l | l | X | l |}
    \hline
    \textbf{Penelitian} & \textbf{Sensor} & \textbf{Fokus \& Metode Pengujian} & \textbf{Analisis Pola} \\
    \hline
    \textcite{latifah2021} & Ultrasonik & \textit{Monitoring} kepenuhan; uji akurasi volume (97\%) & Tidak \\
    \hline
    \textcite{pires2025} & ToF & \textit{Monitoring} real-time urban; uji berbasis skenario & Tidak \\
    \hline
    \textcite{iotsmartwaste2025urban} & Ultrasonik + berat & \textit{Monitoring} + \textit{routing}; \textit{field trial} 30 hari, 12 \textit{bin} (akurasi 85\%) & Tidak \\
    \hline
    \textcite{addas2024waste} & Ultrasonik & \textit{Routing} skala kota; eksperimen data besar tanpa \textit{user testing} & Tidak \\
    \hline
    \textcite{swe2025sustainability} & Ultrasonik & Uji akurasi sensor pada tingkat acuan terkontrol ($\pm$3\%) & Tidak \\
    \hline
    \textbf{Penelitian ini} & ToF (VL53L0X) & Analisis \textbf{pola temporal} kampus; \textit{deployment} 37 hari + evaluasi \textit{requirement} & \textbf{Ya} \\
    \hline
  \end{tabularx}
\end{table}

Dibandingkan dengan penelitian-penelitian tersebut, penelitian ini memiliki perbedaan yang signifikan. Pertama, fokus utama bukan hanya pada \textit{monitoring} kepenuhan, tetapi pada analisis pola pembuangan sampah secara temporal untuk menghasilkan \textit{insight} yang dapat digunakan dalam pengambilan keputusan; sebagaimana terlihat pada Tabel \ref{tab:perbandingan-penelitian}, tidak ada penelitian pembanding yang menjadikan analisis pola sebagai fokus. Kedua, penelitian ini dilakukan pada lingkungan kampus dengan karakteristik yang berbeda dari lingkungan perkotaan umum, dan melibatkan adaptasi \textit{firmware} perangkat pihak ketiga untuk menyesuaikan dengan kebutuhan lingkungan kampus. Ketiga, penelitian ini menggunakan arsitektur IoT 4-\textit{layer} yang lebih komprehensif dengan integrasi \textit{middleware} Node-RED, \textit{database} Supabase, dan \textit{dashboard} berbasis Next.js. Keempat, sebagaimana penelitian terdahulu yang umumnya memvalidasi sistem terhadap kinerja artefak alih-alih penerimaan pengguna, penelitian ini juga menggunakan pendekatan validasi terhadap pemenuhan kebutuhan, sesuai dengan metodologi \textit{Design Science Research}.